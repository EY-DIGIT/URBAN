...
const ViewApplication = () => {
  ...
  const { isLoading: isDownloading, data: downloadData } = Digit.Hooks.death.useDownloadActions({
    tenantId,
    filters: downloadCert ? { id: downloadCert.id } : {},
    auth: true,
    config: {
      enabled: !!downloadCert, // run only when a cert is set
    },
  });

  // Fetch all data on load without filters
  ...
  // Filter data based on searched application ID
  const filteredData = useMemo(() => {
    if (!data?.deathCerts || !searchedId) {
      return data?.deathCerts || [];
    }

    const searchLower = searchedId.toLowerCase();
    return data.deathCerts.filter((cert) => {
      const regNo = (cert.registrationno || "").toLowerCase();
      return regNo.includes(searchLower);
    });
  }, [data, searchedId]);
  ...
  const handleView = (cert) => {
    window.location.href = `/digit-ui/employee/death/view-application/${cert.registrationno}`;
  };

  const handleDownload = (cert) => {
    console.log("Download certificate:", cert.registrationno);
  ...
  useEffect(() => {
    if (downloadData) {
      console.log("Downloaded file data:", downloadData);
      // maybe trigger download here
      setDownloadCert(null); // reset trigger
    }
  }, [downloadData]);
  ...
  const displayData = searchedId ? filteredData : (data?.deathCerts || []);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        {isLoading && <p style={styles.loading}>Loading applications...</p>}
        {isError && <p style={{ color: "red" }}>Error: {error?.message}</p>}

        {!isLoading && !isError && data?.deathCerts && (
          <div style={styles.resultCount}>
            {searchedId
              ? `Showing ${displayData.length} of ${data.deathCerts.length} applications`
              : `Total ${data.deathCerts.length} applications`
            }
          </div>
        )}
        ...
        {!isLoading && !isError && displayData.length === 0 && searchedId && (
          <div style={styles.noResults}>
            No applications found matching Application ID: {searchedId}
          </div>
        )}

        {!isLoading && !isError && data?.deathCerts?.length === 0 && (
          <div style={styles.noResults}>
