...
const EmployeeApp = ({ path, url, userType }) => {
    ...
    const ViewApplication = Digit?.ComponentRegistryService?.getComponent("ViewApplication");
    const DashboardLayout = Digit?.ComponentRegistryService?.getComponent("DashboardLayout");
    const UpdateDeathCertificateApplication = Digit?.ComponentRegistryService?.getComponent("UpdateDeathCertificateApplication");
        ...
        <Switch>
            <React.Fragment>
                <div className="ground-container" style={{ marginTop: "40px", marginLeft: "0px", padding: "0px" }}>
                    ...
                    <PrivateRoute
                        path={`${path}/view-applications`}
                        component={ViewApplication}
                    />

                    <PrivateRoute
                        path={`${path}/view-application/:applicationId`}
                        component={UpdateDeathCertificateApplication}
                    />
                </div>
            </React.Fragment>
        </Switch>
