...
const ServiceCard = ({
  ...
  const handleViewClick = () => {
    ...
    if (getUserType() === "citizen" || title === "Rental") {
      ...
      //  else if(title === "Birth" || title === "Death"){

      //   console.log("OLD  URLLLL=",citizenLink)
      //   const newUrl = citizenLink.replace("/digit-ui", "");
      //   console.log("NEW  URLLLL=",newUrl)
...
{/* <a
  href={
    option?.isExternal
      ? `${baseURL}${citizenLink}?accessToken=${accessToken}&refreshToken=${refreshToken}`
      : (title === "Birth" || title === "Death")
      ? `${window.location.origin}${option.link.replace("/digit-ui", "")}`
      : option.link
  }
...
<a
  href={
    option?.isExternal
      ? `${baseURL}${citizenLink}?accessToken=${accessToken}&refreshToken=${refreshToken}`
      : (title === "Birth" || title === "Death")?option.link?.replace("/digit-ui", "") :  option.link
  }
  style={{ textDecoration: 'none', color: 'inherit' }}
>
                {option?.label}
